...
for question in ("§4.3.2", "How many days must I report a change?", "What changes on 1 March 2026?"):
    print(f"\nQUERY: {question}")
    for result in retriever.retrieve(question, top_k=5):
        label = result.record.provision_no if hasattr(result.record, "provision_no") else result.record.amendment_paragraph
